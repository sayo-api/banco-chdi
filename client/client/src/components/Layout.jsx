...
export default function Layout() {
  const { user, permissions, logout, isAdmin } = useAuth();
  ...
  // Filtrar navegação baseado em permissões (admins veem tudo)
  const visibleNav = NAV.filter(({ to }) => {
    if (isAdmin) return true; // Admins veem tudo
    
    // Mapear rotas para permissões
    if (to === '/') return permissions?.dashboard !== false;
    if (to === '/soldiers') return permissions?.soldiers !== false;
    if (to === '/databases') return permissions?.databases !== false;
    return true;
  });
